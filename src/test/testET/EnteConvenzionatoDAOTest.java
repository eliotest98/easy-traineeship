...
class EnteConvenzionatoDAOTest {
  ...
  EnteConvenzionato ente = new EnteConvenzionato("azienda@email.it", "Cap Gemini", "NA", 'N',
      "password", 3, "25/12/1980", "99999999999", "Salerno", "Giacomo", "Carmine", "3401414140", 8,
  ...
  void testAllEnte() throws SQLException {
    ...
    try {
      ...
      String sql2 =
          ("INSERT INTO EnteConvenzionato VALUES('99999999999',"
              + "'Salerno','Giacomo','3490000141','100','Carmine',"
              + "'Francesco','12/10/1980','Molto interessante','azienda@email.it');");
    ...
    for (i = 0; i < listaEnti.size(); i++) {
      if (listaEnti.get(i).getEmail().equals(ente.getEmail())
          && listaEnti.get(i).getPartitaIva().equals(ente.getPartitaIva())) {
        inserito = true;
      }
    }
  ...
  void testRicercaEnteByPartitaIva() throws SQLException {
    ...
    if (("azienda@email.it").equals(ente.getEmail())
        && ("99999999999").equals(ente.getPartitaIva())) {
      trovato = true;
    }
  ...
  void testRicercaEnteByEmail() throws SQLException {
    ...
    try {
      ...
      stmtSelect.executeUpdate(sql1);
      String sql2 =
          ("INSERT INTO EnteConvenzionato VALUES('99999999999','Salerno',"
              + "'Giacomo','3490000141','100','Carmine','Francesco',"
              + "'12/10/1980','Molto interessante','azienda@email.it');");
      stmtSelect.executeUpdate(sql2);
      conn.commit();
    } catch (Exception e) {
      e.printStackTrace();
  ...
  // Test del metodo ModificaEnte di EnteConvenzionatoDAO quando l'elemento da modificare non è
  ...
  // Test del metodo ModificaEnte di EnteConvenzionatoDAO quando l'elemento da modificare è presente
  // nel database
  @Test
  void testModificaEnteCampoPresente() throws SQLException {
    ...
    try {
      ...
      String sql2 =
          ("INSERT INTO EnteConvenzionato VALUES('99999999999',"
              + "'Salerno','Giacomo','3490000141','100','Carmine',"
              + "'Giuseppe','12/10/1975','Molto interessante','azienda@email.it');");
      stmtSelect.executeUpdate(sql2);
      conn.commit();
    } catch (Exception e) {
      e.printStackTrace();
