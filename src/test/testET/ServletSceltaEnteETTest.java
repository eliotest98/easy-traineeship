...
class ServletSceltaEnteETTest {
  ...
  void testCampoEnteTroppoLungo() {
    when(requestMock.getParameter("ente"))
        .thenReturn("Carlo SRLSRLSRLSRLSRLSRLSRLSRLSRLSRLSRLSRLSRLSRLSRLSRLSRLSRLSRLSR");
    ServletSceltaEnteET test = new ServletSceltaEnteET();
    IllegalArgumentException e =
        assertThrows(IllegalArgumentException.class, () -> test.doPost(requestMock, responseMock));
  ...
  void testCampoNomeTroppoLungo() {
    when(requestMock.getParameter("ente")).thenReturn("11111111111");
    when(requestMock.getParameter("nome"))
        .thenReturn("MarioMarioMarioMarioMarioMarioMarioMarioMarioMarioMarioMario");
    ServletSceltaEnteET test = new ServletSceltaEnteET();
    IllegalArgumentException e =
        assertThrows(IllegalArgumentException.class, () -> test.doPost(requestMock, responseMock));
  ...
  void testCampoCognomeTroppoLungo() {
    when(requestMock.getParameter("ente")).thenReturn("11111111111");
    when(requestMock.getParameter("nome")).thenReturn("Mario");
    when(requestMock.getParameter("cognome"))
        .thenReturn("RossiRossiRossiRossiRossiRossiRossiRossiRossiRossiRossiRossi");
    ServletSceltaEnteET test = new ServletSceltaEnteET();
    IllegalArgumentException e =
        assertThrows(IllegalArgumentException.class, () -> test.doPost(requestMock, responseMock));
  ...
  void testCampoFacoltaTroppoLunga() {
    ...
    when(requestMock.getParameter("nome")).thenReturn("Mario");
    when(requestMock.getParameter("cognome")).thenReturn("Rossi");
    when(requestMock.getParameter("facolta"))
        .thenReturn("InformaticaInformaticaInformaticaInformaticaInformatica");
    ServletSceltaEnteET test = new ServletSceltaEnteET();
    IllegalArgumentException e =
        assertThrows(IllegalArgumentException.class, () -> test.doPost(requestMock, responseMock));
  ...
  void testCampoDescrizioneTroppoLungo() {
    ...
    when(requestMock.getParameter("facolta")).thenReturn("Informatica");
    when(requestMock.getParameter("descrizione")).thenReturn(
        "La comprensione, nella pratica, di quali sono i contenuti "
        + "e le logiche di un sistema di controllo di gestione: il sistema "
        + "direzionale, l’efficacia, l’efficienza, nonché gli attori in "
        + "gioco e l'utilità che reca tale funzione all'azienda nel suo "
        + "complesso.aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
    ...
    try {
      ...
      String sql3 =
          ("INSERT INTO enteconvenzionato VALUES('11111111111',"
              + "'Avellino','Salvatore Totti','0825519149','100','Michele Persico',"
              + "'Michele Porto','08/01/1977','esperti in siti web','green@gmail.com');");
      stmtSelect.executeUpdate(sql3);
      String sql1 = ("INSERT INTO tirocinio VALUES('1','" + modifiedDate
          + "','11','informatica','javascript','Java','Bene','Accettato',"
          + "'progettoformativa.pdf','ragazzo valido','4859',NULL);");
      stmtSelect.executeUpdate(sql1);
      conn.commit();
      when(requestMock.getParameter("ente")).thenReturn("11111111111");
      when(requestMock.getParameter("nome")).thenReturn("Mario");
