...
public class EnteConvenzionatoDAO {
  ...
  /**
   * Metodo che interroga il DB e restituisce tutti gli 'EntiConvenzionati' persenti all' interno.
   * 
   * @return listaEnti
   */
  public synchronized ArrayList allEnte() throws SQLException {
    ...
    try {
      ...
      ps = con.prepareStatement("SELECT * " + "FROM ENTECONVENZIONATO, USER "
  ...
  /**
   * Metodo che interroga il DB e restituisce l' 'EntiConvenzionato' persento all' interono tramite
   * l'email.
   * 
   * @return purchase
   */
  public synchronized EnteConvenzionato ricercaEnteByPartitaIva(String partitaIva)
      throws SQLException {
  ...
  /**
   * Metodo che interagisce con il DB e inserisce in esso l' 'EnteConvenzionato' passato come
   * parametro.
   * 
  ...
  public synchronized boolean inserisciEnte(EnteConvenzionato enteConvenzionato)
    ...
    PreparedStatement psEnteConvenzionato = null;// Creazione oggetto Statement per
    // l'EnteConvenzioanto
    ...
    try {
      ...
      psUser =
          con.prepareStatement("INSERT INTO USER(EMAIL, NAME, SURNAME, SEX, PASSWORD, USER_TYPE) "
              + "VALUES (?, ?, ?, ?, ?, 3)");
      ...
      psEnteConvenzionato =
          con.prepareStatement("INSERT INTO ENTECONVENZIONATO(PARTITAIVA, SEDE, RAPPRESENTANTE,"
  ...
  /**
   * Metodo che interagisce con il DB e modifica l' 'EnteConvenzionato' di riferimento.
   * 
   * @param enteConvenzionato
   * 
   * @return boolean
    ...
    try {
      ...
      if (update == false) {
        ...
        psUser = con.prepareStatement(
            "UPDATE USER " + ("SET EMAIL=?, NAME=?, SURNAME=?, SEX=?") + ("WHERE email = ?;"));
        psUser.setString(1, enteConvenzionato.getEmail());
        psUser.setString(2, enteConvenzionato.getName());
        psUser.setString(3, enteConvenzionato.getSurname());
        psUser.setString(4, "N");
        ...
        psEnteConvenzionato = con.prepareStatement(
            "UPDATE ENTECONVENZIONATO " + "SET SEDE = ?, RAPPRESENTANTE = ?, TELEFONO = ?,"
                + "DIPENDENTI = ?, DOTRIFERIMENTO = ?, " + "REFERENTE = ?, DATANASCITA = ?, "
                + "DESCRIZIONEATTIVITA = ?, EMAIL = ? " + "WHERE PARTITAIVA=?;");
  ...
  /**
   * Metodo che interagisce con il DB ed elimina l' 'EnteConvenzionato' di riferimento.
   * 
   * @param email
   * 
   * @return boolean
  ...
  /**
   * Metodo che interagisce con il DB e modifica la 'password' dell' 'EnteConvenzionato' di
   * riferimento.
   * 
   * @param email
   * 
