...
public class EnteConvenzionatoDAO {
	...
	public synchronized ArrayList allEnte() {
		...
		try {
			...
			ps = con.prepareStatement(
					"SELECT * " + "FROM ENTECONVENZIONATO, USER " + "WHERE ENTECONVENZIONATO.EMAIL=USER.EMAIL");
	...
	public synchronized boolean inserisciEnte(EnteConvenzionato enteConvenzionato) {
		...
		try {
			...
			psUser = con.prepareStatement(
					"INSERT INTO USER(EMAIL, NAME, SURNAME, SEX, PASSWORD, USER_TYPE) " + "VALUES (?, ?, ?, ?, ?, 3)");
			...
			// Insert per l'inserimento in 'EnteConvenzionato' dei dati parziali di
			// 'EnteConvenzionato'
			psEnteConvenzionato = con.prepareStatement("INSERT INTO ENTECONVENZIONATO(PARTITAIVA, SEDE, RAPPRESENTANTE,"
					+ " TELEFONO, DIPENDENTI, DOTRIFERIMENTO, REFERENTE," + "DATANASCITA, DESCRIZIONEATTIVITA, EMAIL) "
					+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);");
	...
	/*
	 * Metodo che interagisce con il DB e modifica l' 'EnteConvenzionato' di
	 * riferimento
	 * 
	 * @param enteConvenzionato
	 * 
	...
	public synchronized boolean modificaEnte(EnteConvenzionato enteConvenzionato) {
		...
		try {
			...
			if (update == false) {
				...
				psUser = con.prepareStatement(
						"UPDATE USER " + ("SET EMAIL=?, NAME=?, SURNAME=?, SEX=?, PASSWORD=? ") + ("WHERE email = ?;"));
				psUser.setString(1, enteConvenzionato.getEmail());
				psUser.setString(2, enteConvenzionato.getName());
				psUser.setString(3, enteConvenzionato.getSurname());
				psUser.setString(4, " ");
				...
				// Update per la Modifica in 'EnteConvenzionato' dei dati parziali di
				// 'EnteConvenzionato'
	...
	/*
	 * Metodo che interagisce con il DB e modifica la 'password' dell'
	 * 'EnteConvenzionato' di riferimento
	 * 
	 * @param email
	 * 
	 * @param password
	...
	public synchronized boolean updatePassword(String email, String password) {
		...
		try {
			...
			psUser = con
					.prepareStatement("UPDATE USER " + "SET PASSWORD =" + password + " WHERE EMAIL = '" + email + "';");
			// Se la modifica va a buon fine restituisce true
