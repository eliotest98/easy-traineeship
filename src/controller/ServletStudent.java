	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (conn != null) {
			...
			if (flag == 1) { // registrazione nuovo utente
				...
				/*
				 * l'email � valida se la sua lunghezza � diversa da 0, se non � presente
				...
				/*
				 * da decidere se tenere o togliere String graduation =
				...
				try {
					sql = " SELECT id_request " + "FROM request WHERE fk_user = ? AND fk_state != ? AND fk_state != ? ";
					stmt = conn.prepareStatement(sql);
					...
					if (r.wasNull()) {
						...
						int count = r.last() ? r.getRow() : 0;
						if (count == 0) {
							sql = " INSERT INTO request " + " (level, release_date, expiry_date, year, requested_cfu,"
